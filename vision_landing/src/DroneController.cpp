...
namespace vision_landing
{
        ...
        : nodeHandle_(nodeHandle)
    {
        ...
        homePosSub_ = nodeHandle_.subscribe<mavros_msgs::HomePosition>("/mavros/home_position/home", 1, &DroneController::homeCallback, this);
        ...
        setHomeClient_ = nodeHandle_.serviceClient<mavros_msgs::CommandHome>("/mavros/cmd/set_home");
    ...
    void DroneController::homeCallback(const mavros_msgs::HomePosition::ConstPtr &msg)
    {
    ...
    */
    ...
    bool DroneController::startVisionLanding()
    {
        ...
        //acquire home position after arming
        mavros_msgs::HomePosition homePos = homePosMsg_;
    ...
    void DroneController::land()
    {
        ...
        while (ros::ok)
        {
            ...
            {
                outputZ = 0;
            } // if last pose is less than 1.1 then althold

            double PID_LIMIT_XY = 1.400;

            if (outputX > PID_LIMIT_XY)
            {
                outputX = PID_LIMIT_XY;
            }
            if (outputX < -PID_LIMIT_XY)
            {
                outputX = -PID_LIMIT_XY;
            }
            if (outputY > PID_LIMIT_XY)
            {
                outputY = PID_LIMIT_XY;
            }
            if (outputY < -PID_LIMIT_XY)
            {
                outputY = -PID_LIMIT_XY;
